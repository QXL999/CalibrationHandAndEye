...
public:
	...
	std::vector<_Ellipse> DetectImage(const cv::Mat3b &image);
	std::vector<_Ellipse> operator()(const cv::Mat3b& image);
	...
	void EnumerateArcs();
	_Ellipse FitEllipses(const std::vector<int>& arc_ids,const VVP& arcs, double cover = 0.4);
	double ValidateEllipse(const _Ellipse&ell, const VVP& arcs,double cover);
	...
	double EllipseDistance(const _Ellipse& a, const _Ellipse& b);
